...
class Deepligand3D:
    ...
    def __init__(self, list_of_model_paths, device='cpu', **kw_args):
        ...
        assert isinstance(list_of_model_paths, list)
        assert len(list_of_model_paths) != 0, 'no networks path given'

        self.COFACTORS_STD = [f'{cof}_std' for cof in self.COFACTORS]
        self.CLASSES = len(self.COFACTORS)
        self.device_type=device
        self.list_of_model_paths = list_of_model_paths
        if self.device_type == 'cuda':
            if not torch.cuda.is_available():
                raise KeyError('no cuda available')
        self.device = torch.device(self.device_type)
        
        self.model_list = torch.nn.ModuleList()
        
        for path in self.list_of_model_paths:
